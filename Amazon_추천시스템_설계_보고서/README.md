...
$$\text{보정 평점} = \frac{n \times r + m \times C}{n + m}$$
...
리뷰 20만～43만 건 상품은 보정 폭이 0.01 미만으로 자기 평점을 거의 유지한다. 반면 평점 4.8이지만 리뷰가 3,964건인 에어프라이어는 0.38점이나 하향 조정되어, 원 평점만으로는 1위였을 상품이 26위로 내려갔다. 표본 신뢰도가 순위에 반영된 것이다.
...
$$\text{value\_gap} = \text{평점 백분위} - \text{가격 백분위}$$
...
상위 30건이 19개 카테고리에 분산되었고, `value_gap` 은 0.400～0.909로 충분히 벌어져 순위가 유의미하다.
...
기준 카테고리 6종, 보완 사유 8종으로 분산된 결과가 산출되었다.
...
-- 2. 품질 검증
SELECT (SELECT count(*) FROM amazon)         AS 원본,
